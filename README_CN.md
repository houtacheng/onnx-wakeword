...
> 表中数字测于**困难负样本留出集**（语音+音乐+混音，专门挑选的易误触发内容）——这是**最坏情况**口径。在干净的公开朗读语料 AISHELL-1 上实测更低（你好小娜：基线 278.7 → 优化版 0.6 次/小时）。

> 以上均为**裸模型**口径（检测层全关）。运行时的防误触发检测层可以继续压低——仅开启 L1 连续帧（默认即开）后，优化版你好小娜在同一份 10 小时中文语音/音乐语料上从 10.3 降到 **4.9 次/小时**，这还没叠加其他层（L3/L5 等，按需再开）。
...
```text
我的模型 → 误触发优化 → 负样本扫描 → 全量重训 → 完成后下载 R1 模型
```
